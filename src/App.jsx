...
const App = () => {
  ...
  useEffect(() => {
    ...
    if (stage === 'deckTimer') {
      if (!isTimerActive) {
        // 6 second delay before the timer starts
        delayTimeout = setTimeout(() => {
          setIsTimerActive(true);
        }, 6000); 
      } else if (timeLeft > 0) {
        // Start playing the alarm sound when timer starts
        try {
  ...
  const handleApologyRecover = () => {
    if (previousStage === 'questions' || previousStage === 'confirmation') {
        // Go back one question step
        if (questionStep > 0) {
            setQuestionStep(questionStep - 1);
            setStage('questions');
        } else {
            // If on the first question, go back to the deal number stage
            setStage('deal_number'); 
        }
    } else if (previousStage === 'ask_specific_card') {
        // Go back to the final question stage
        setStage('questions');
    } else if (previousStage) {
        // For other stages, just go back
  ...
  const handleProceedToNextQuestion = () => {
      // Check if this was the last step in the flow
  ...
  const questionFlow = [
    ...
    {
      // This Q is conditional on the color attribute being set
    ...
    // This is only included if the card type is 'Number'
  ...
  const renderTrickSelect = () => (
    <ScriptView onNext={() => setStage('choose_number_mode')}>
      ...
      <p className="text-3xl md:text-4xl text-red-400 font-bold mb-6 leading-tight">
        "Make sure I do not see it, and make sure <span className="underline decoration-[#D4C5B0] underline-offset-4">nobody here</span> sees it."
      </p>
      <p className="text-xl text-emerald-400 italic mb-6 font-light border-l-2 border-emerald-500 pl-4">
            ...
            <div className="bg-[#1a1a1a] p-6 border-l-4 border-[#D4C5B0]">
                ...
                <button
                    onClick={() => {
                        ...
                        if(val && val > 0 && val < 53) {
                            setTargetNumber(parseInt(val, 10)); // Ensure it's an integer
                            setStage('deal_number');
                        }
                    }}
  ...
  const renderDealNumber = () => (
    <ScriptView onNext={() => setStage('questions')}>
      ...
      <div className="bg-[#1a1a1a] p-8 border-l-4 border-[#D4C5B0] mb-8 text-center shrink-0">
        ...
        <div className="text-8xl font-bold text-white font-['Poppins'] tracking-tighter">
          {targetNumber}
        </div>
      </div>
  ...
  const renderQuestions = () => {
    ...
    const handleAnswer = (option) => {
      // 1. Perform the action (update state)
      setCardAttributes({ ...cardAttributes, [option.key]: option.value });
      
      // 2. Set the confirmation joke and stage
      setConfirmationText(option.joke);
      setStage('confirmation');

      // 3. Play the magic sound
      playMagicSound(); 
    };
  ...
  const renderConfirmation = () => (
    <div className="flex flex-col h-full max-w-2xl mx-auto px-6 py-8 justify-center items-center text-center animate-fadeIn relative font-['Poppins']">
        ...
        <p className="text-5xl text-red-400 font-bold mb-12 leading-tight">
            "{confirmationText}"
        </p>
        <button
  ...
  return (
    <div className="min-h-screen bg-[#111111] text-white overflow-hidden font-sans flex flex-col selection:bg-[#D4C5B0] selection:text-black">
      ...
      {/* Persistent Header Removed per request */}
      ...
      {/* Progress Bar (Bottom) - Hidden on setup/finale */}
      {!['setup', 'finale', 'apology', 'confirmation'].includes(stage) && (
        <div className="fixed bottom-0 left-0 w-full h-2 bg-black z-50">
          <div 
            ...
            style={{ 
              width: `${['instructions', 'intro', 'deckCheck', 'deckTimer', 'trick_shuffle', 'trick_select', 'choose_number_mode', 'deal_number', 'questions', 'ask_specific_card'].indexOf(stage) * 10}%` 
            }}
          />
        </div>
      )}
